...
A complete toolkit for voice cloning, guided meditation generation, song transcription, and audio processing — built around [Coqui XTTS v2](https://github.com/coqui-ai/TTS) with a Tkinter GUI that unifies every script under one roof.
...
Every tool is accessible through a single Tkinter interface (`xtts_studio.py`) or, if you prefer, directly from the command line.
...
- **Inline XTTS parameters** per voice using curly-brace syntax: `{N, seed, trim_start, trim_end, fade_in, fade_out, temperature, top_k, top_p}`
- **Per-voice audio configuration** using bracket syntax: `[N, LANG, speed, volume, eq_low, eq_mid, eq_high, hp, lp, noise_reduction, compression, de-esser]`
...
Fast and accurate audio-to-text transcription powered by [faster-whisper](https://github.com/SYSTRAN/faster-whisper):
...
- Python 3.10
- [Miniconda](https://docs.conda.io/en/latest/miniconda.html) or Anaconda (recommended for environment isolation)
...
**ZFEbHVUE** — [GitHub](https://github.com/ZFEbHVUE)
