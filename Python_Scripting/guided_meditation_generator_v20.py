...
# -*- coding: utf-8 -*-
"""
Guided Meditation Generator - VERSION 20 - PER-VOICE XTTS INLINE PARAMS
========================================================================
- NEW v20: Inline XTTS params in curly-brace block {N, seed, trim_start, trim_end,
...
TWO bracket formats in the text file:
  ...
  2) XTTS-PARAMS block (curly braces, exactly 9 values):
     {N, seed, trim_start, trim_end, fade_in, fade_out, temp, top_k, top_p}
      ↑   ↑       ↑           ↑        ↑        ↑        ↑     ↑      ↑
      N  seed  trim_ms    trim_ms    ms       ms       0-1   int   0-1

       Value 0 meaning ({N, 0, 0, 0, 0, 0, 0, 0, 0}) :
...
Typical usage (both brackets together):
  {1, 42, 110, 255, 150, 300, 0.65, 50, 0.85}              ← XTTS params voice 1
  [1, FR, 0.85, +1, -5, +1, -1, 90, 9000, 0.3, 0.4, 0.2]  ← audio params voice 1
  Text for french voice 1...

  {2, 42, 60, 339, 150, 300, 0.60, 40, 0.80}               ← XTTS params voice 2
  [2, EN, 0.85, +8, -5, +1, -1, 90, 9000, 0.5, 0.5, 0.2]  ← audio params voice 2
  Text for english voice 2...
...
Pause syntax:
 ...
 These king of pauses can be used manually in you prompt.txt 
...
def parse_xtts_params(segment, base_params):
    ...
    Example: {1,0,0,0,0,0,0,0,0}
      → voice 1, no seed, no trim, no fades, XTTS defaults
...
def generate_meditation(text, output_file, voice_files, ambient_file, music_files):
    ...
    temp_file          = "/tmp/temp_tts_v20.wav"
    music_to_apply = []
    position_ms        = 0
...
def main():
    if len(sys.argv) < 4:
        print("Usage: python guided_meditation_generator_v20.py script.txt output.wav voice1.wav [voice2.wav ...] [music...]")
        print("\nVERSION 20 - Per-voice XTTS params, multi-language support")
        ...
        print("\nXTTS params block (curly braces):")
        print("  {N, seed, trim_start, trim_end, fade_in, fade_out, temp, top_k, top_p}")
        print("  {1, 42, 110, 255, 150, 300, 0.65, 50, 0.85}")
        print("  {1, 0, 0, 0, 0, 0, 0, 0, 0}         # no trim, no fades, XTTS defaults")
